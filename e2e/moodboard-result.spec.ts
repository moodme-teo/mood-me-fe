 ...
 *   무드 성향 그래프 — 5축 시각화 (고요↔활기 · 따뜻↔서늘 · 미니멀↔맥시멀 · 빈티지↔모던 · 현실↔몽환)
 ...
 *
 * 시나리오:
 * - 편집을 마친 사용자가 무드보드 캔버스와 무드 성향 이름을 확인한다.
 * - "이미지 내보내기" 를 누르면 PNG 다운로드가 일어난다 (§5.6 — 캔버스를 파일로 렌더).
 * - "SNS 공유" 를 누르면 링크를 클립보드에 복사하고 토스트를 띄운다.
 * - "편집" 을 누르면 재편집 화면으로 왕복한다.
 * - 무드보드를 불러오지 못하면 에러 화면을 보여준다.
 *
 * 테스트 성격: smoke (+ 로드 실패는 edge case)
 *
 * 전제 조건:
 ...
 *
 * 테스트하지 않는 것:
 * - 무드 성향 5축 그래프 — §5.6 이 "그래프 표현 방식(종류·개수)은 아직 확정이 아님" 이라
 *   명시한다. 레이더 차트와 막대 스펙트럼 두 안이 열려 있어 단언할 대상이 없다.
