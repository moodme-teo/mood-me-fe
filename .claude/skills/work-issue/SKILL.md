---
name: work-issue
description: docs/work 안의 작업 문서를 읽어 GitHub 이슈(시안 A - 이모지 브래킷형)로 자동 등록하고, 문서 작성자에게 배정한 뒤 "무드미 MVP" 프로젝트 보드(백로그)에 추가합니다. 라벨은 페이지 단위로 붙습니다. 개발자가 작업 명세를 docs/work에 적은 뒤 이슈를 만들 때 사용합니다.
---

# /work-issue — 작업 문서 → GitHub 이슈 자동 등록

`docs/work/`에 작성된 작업 문서를 GitHub 이슈로 등록한다. 이슈는 **시안 A(이모지 브래킷형)** 컨벤션을 따르며, **문서를 작성한(= 이 스킬을 실행한) 사람**에게 자동 배정하고 **`무드미 MVP` 프로젝트 보드의 `백로그`** 에 올린다.
...
`docs/work/` 에서 `_template.md`, `README.md` 를 **제외한** `.md` 파일 중, front-matter의 `issue:` 값이 **비어있는**(아직 미등록) 문서만 후보로 모은다.

```bash
ls docs/work/*.md 2>/dev/null | grep -vE '/(_template|README)\.md$'
```

각 후보의 front-matter에서 `issue:` 를 확인:
- `issue:` 에 이미 숫자가 있으면 → 등록 완료된 문서. 후보에서 제외.
- 후보가 **0개** → "등록할 새 작업 문서가 없습니다. `docs/work/`에 `_template.md`를 복사해 작성한 뒤 다시 실행하세요." 안내 후 중단.
- 후보가 **1개** → 그 문서로 진행.
...
- **이슈 본문** = 문서의 `## 📋 개요` / `## ✅ 작업 내용` / `## 🎯 완료 조건` / `## 🔗 참고` 섹션을 그대로 사용하되, 참고 섹션 맨 위에 원본 문서 경로 한 줄을 추가한다:
  - `## 🔗 참고` 첫 항목에 `- \`docs/work/<파일명>.md\`` 를 넣는다(중복이면 생략).
