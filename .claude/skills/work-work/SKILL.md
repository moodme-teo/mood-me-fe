---
name: work-work
description: 나에게 배정된 열린 GitHub 이슈 목록을 불러와 그중 작업할 것을 선택하면 작업을 시작합니다. 레포 결과물이 있는 작업(output repo)은 dev 기준 브랜치를 따 draft PR을 즉시 열어 구현·커밋을 이어가고, 완료 조건 충족 시 ready for review 로 전환해 리뷰어를 지정합니다. 레포 결과물이 없는 작업(output none)은 PR 없이 이슈로만 추적합니다. 배정받은 작업을 시작할 때 사용합니다.
---

# /work-work — 배정된 이슈 선택 → (repo면) draft PR → 구현 → ready

나에게 배정된 이슈 중 하나를 골라 작업을 시작한다. **PR은 레포에 들어갈 결과물이 있는 작업(`output: repo`)에만 연다.** 진행중 표시를 위해 **작업 시작 시점에 draft PR을 미리 열고**, 구현하며 커밋을 쌓다가 완료 조건을 충족하면 ready for review 로 전환한다. 리뷰가 끝나면 `/work-done` 으로 머지·완료 처리한다. 보드 상태 이동은 GitHub Projects 자동화가 처리하므로 직접 옮기지 않는다.
...
- **`none`** → 브랜치/PR 없이 여기서 종료. "레포 결과물이 없는 작업이라 PR 없이 이슈로만 추적합니다. 작업이 끝나면 `/work-done` 으로 완료 처리하세요." 안내하고 결과 보고(6)로 간다.
- **`repo`** → 아래 계속. **페이지 라벨**을 확보한다(PR에 그대로 붙임):
...
## 3. 작업 브랜치 생성 + draft PR 즉시 오픈 (`output: repo`)
...
- **리뷰어는 draft 단계에서 지정하지 않는다** — ready for review 전환 시(5단계) 지정.
...
## 5. 완료 조건 충족 → ready for review + 리뷰어 지정
...
이후 **리뷰**가 끝나면 사용자가 `/work-done` 을 실행해 **squash-merge + 이슈 종료**까지 마무리한다. 보드 상태는 GitHub Projects 자동화가 옮긴다.

## 6. 결과 보고

- (`output: repo`) 🌿 브랜치: `feat/login` (dev 기준) · 🔀 draft PR: URL (base: dev, `Closes #N`) · 🏷 라벨: <페이지>
- (`output: repo`, ready 전환까지 한 경우) 👀 리뷰어: <본인 제외 협업자>
- (`output: none`) PR 없음 — 이슈로만 추적
- 다음 단계: 리뷰 → `/work-done` (머지 + 이슈 닫기, 보드는 자동)

> `/work-work` 는 절대 `dev`/`main`에 직접 push하거나 머지하지 않는다. 머지는 리뷰가 끝난 뒤 `/work-done` 으로 진행한다.
