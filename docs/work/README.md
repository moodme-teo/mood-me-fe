...
무드미 팀은 **todo 등록 → AI 이슈화 → 배정 → 작업 시작(진행중) → 구현/PR → 리뷰 → 머지 → 완료 처리** 흐름으로 작업합니다.
...
/work-work ──▶ 보드 '진행중'
   ├─ output: repo ──▶ 브랜치 + draft PR 즉시 오픈 ──▶ 구현·커밋 ──▶ ready for review ──▶ 리뷰 ──▶ 머지(사람) ──▶ /work-done (보드 '작업 완료')
   └─ output: none ──▶ PR 없음, 이슈·보드로만 추적 ──────────────────────────▶ 작업 끝 ──▶ /work-done (이슈 닫기 + 보드 '작업 완료')
```

> **PR 정책**: PR은 **레포에 들어갈 결과물(코드·문서·설정 등)이 있는 작업에만** 엽니다. 신청·회의·이름 선정처럼 레포 밖에서 끝나는 작업은 PR 없이 이슈 + 보드로만 관리합니다. repo 작업은 진행중 표시를 위해 **작업 시작 시 draft PR을 미리 열고**, 커밋을 쌓다가 완료 조건 충족 시 ready for review 로 전환합니다.
...
## 3. `/work-work` — 작업 시작 (진행중 + draft PR)
...
- `output: none` → 이슈를 닫고(completed) 보드를 **`작업 완료`** 로 이동
- `output: repo` → 연결 PR **머지 확인 후** 보드를 **`작업 완료`** 로 이동 (미머지면 중단·안내. 이슈는 머지 시 `Closes #N` 로 자동 닫힘)

> 💡 Projects 내장 workflow **"Item closed → 작업 완료"** 를 보드 설정(UI)에서 한 번 켜두면 이슈가 닫힐 때 보드 이동이 자동화됩니다.

## 4. 일정관리 · 시각화

별도 인프라 없이 **GitHub Projects(무드미 MVP)** 를 사용합니다.

- **상태**: Board 뷰 — 백로그 → 진행중 → 작업 완료 (`/work-work` 시작 시 진행중, `/work-done` 완료 시 작업 완료로 이동)
