...
무드미 팀은 **todo 등록 → AI 이슈화 → 배정 → 구현 → PR → 리뷰 → 머지** 흐름으로 작업합니다.
todo 등록, 이슈 등록, 구현/PR 을 세 개의 스킬로 자동화했습니다.

```
/work-todo ──▶ todo-list.md ──/work-issue a1──▶ 이슈 문서 초안 + GitHub 이슈(배정+라벨+보드+일정) ──/work-work──▶ 구현+커밋+PR ──▶ 리뷰 ──▶ 머지
 (번호 자동 부여)  (사람이 관리)                  (docs/work/todo/)
```
...
- 연결된 `docs/work/todo` 명세대로 구현 (`dev` 기준 새 브랜치)
