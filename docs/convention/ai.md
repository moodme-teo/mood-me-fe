...
- AI 응답(이미지 프롬프트 + 키워드 9개 + 무드 프로파일)은 **구조화해서** 받습니다: tool use(JSON 강제) 또는 텍스트 응답을 **Zod 스키마로 파싱**. 응답 텍스트를 정규식으로 긁는 코드 금지.
...
- 모든 AI 호출에 타임아웃 설정 — 상수로 관리 (예: 텍스트 30s, 이미지 60s). 초과 → `AI_TIMEOUT`.
...
- 부분 실패 정책: 키워드는 나왔는데 이미지가 실패하면 job은 `failed`로 처리하되 재시도 시 텍스트 분석 결과를 재사용할 수 있게 저장합니다.

## 보안 / env

- `ELICE_MODEL_API_KEY`, `ELICE_BASE_URL`, `SUPABASE_SERVICE_ROLE_KEY`는 서버 전용 — **`NEXT_PUBLIC_` 접두 금지**, 클라이언트 번들에 절대 노출하지 않습니다.
- `lib/elice-ai.ts`는 클라이언트에서 import 금지 (`server-only`로 강제됨).
