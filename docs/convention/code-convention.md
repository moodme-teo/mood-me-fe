...
- **도메인 어휘는 PRD §7 데이터 모델을 따릅니다**: `Moodboard`, `MoodboardElement`, `MoodTestSession`, `MoodTestAnswer`, `MoodboardGenerationJob`, `GuestSession`. 같은 개념에 새 이름을 만들지 않습니다.
...
- 상태 관리 라이브러리(zustand, TanStack Query 등)는 **현재 도입하지 않습니다.** 필요가 증명되면(예: 캔버스 요소 상태가 prop drilling 한계) 팀 합의 후 도입.
...
- 프롬프트 템플릿은 `lib/prompts.ts`(추가 예정)에 모아 코드와 분리합니다.
...
- 조건부 클래스 조합이 복잡해지면 그때 `clsx` 계열 도입을 논의합니다 (현재 미도입).
