...
**시각 언어의 축:** 순수 화이트 캔버스 위에서, 깊이는 검정 그림자가 아니라 **색조 그림자(color-tinted shadow)**로 표현된다. 색은 flat 단색이 아니라 **글로우 그라디언트**(hue+명도 대비를 가로지르는 2-stop)로 쓰인다. UI는 hover 시 위로 떠오르고 press 시 눌리는, 물리적으로 "만져지는" 모션을 가진다.
...
무드가 색으로 표현되는 제품이므로 팔레트가 브랜드의 핵심 자산이다. 색 전략은 **Restrained 크롬 + 결과물이 색을 주도**: 앱 크롬은 화이트 캔버스와 뉴트럴로 절제하고, 밝은 팔레트는 CTA·강조·상태에만 의도적으로 등장시켜 무드보드 결과물을 돋보이게 한다.

### Neutral scale (cool gray, 따뜻한 잉크 아님)

| 토큰         | 값        | 용도                                                  |
| ------------ | --------- | ----------------------------------------------------- |
| `--gray-900` | `#16161a` | 본문 잉크 (`--text-primary`)                          |
| `--gray-700` | `#43434a` | 강한 보조 텍스트                                      |
| `--gray-500` | `#7b7b84` | 보조 텍스트 (`--text-secondary`)                      |
| `--gray-300` | `#d2d2d8` | 흐린 텍스트/구분선 (`--text-muted`)                   |
| `--gray-100` | `#edeef2` | 테두리/placeholder (`--border`, `--placeholder-fill`) |
| `--gray-050` | `#f6f7fa` | sunken 표면 (`--surface-sunken`)                      |

표면: `--surface-page` / `--surface-card` = `#ffffff`, `--surface-inverse` = `--gray-900`. AI 클리셰(따뜻한 near-white 크림/샌드)는 회피한다.
...
**언어별 폰트 규칙(핵심):** 한국어·일반 UI 텍스트는 **SUIT Variable**(산세리프 전용, 예외 없음). 영문/라틴 **display** 단어(예: "Vision", "Vibe")만 **Instrument Serif** 악센트. 세리프는 영문 전용 — 한글 문자에는 절대 적용하지 않는다.

- `--font-body`: `"SUIT Variable", -apple-system, "Apple SD Gothic Neo", "Malgun Gothic", sans-serif` — 한 문자열 안에서 라틴은 우선 폰트, 한글은 SUIT로 자동 분리(mixed stack)
- `--font-display-en`: `"Instrument Serif", "Times New Roman", serif` — 영문 display 전용
...
- **Display (KR/mixed, SUIT heavy):** `text-display-xl` 800·84px … `text-display-sm` 700·32px. 결과 공개·히어로 순간. tracking `-0.02em`.
- **Display (EN accent, Instrument Serif):** `text-display-en-xl` italic·88px … `text-display-en-sm` italic·34px. 영문 단어 전용.
- **Heading:** `text-heading-lg` 700·28px, `text-heading-md` 600·22px. 섹션·질문 제목.
- **Body:** `text-body-lg` 500·18px, `text-body-md` 400·16px(본문 기본), `text-body-sm` 400·14px.
- **Label / Caption:** `text-label` 600·13px(버튼·칩·메타), `text-caption` 400·12px.
...
- `--shadow-card` / `--shadow-card-hover`: 중립 카드 그림자(쿨 바이올렛-그레이 틴트)
...
| `Card`     | 흰 카드 + 색조 그림자(테두리 없음), `tone` 옵션. Header/Title/Description/Content/Footer 조합.                                                                             |
...
텍스트 마크(로고 파일 없음): `mood` 는 `--text-primary`(잉크), `me` 는 `--violet-500`, 둘 다 `--font-body` 800 weight. 예: **mood**<span style="color:#8b5cf6">me</span>.
...
- **Do** 한글은 SUIT, 영문 display 단어만 Instrument Serif.
...
- **Don't** 한글에 세리프(Instrument Serif)를 적용한다.
