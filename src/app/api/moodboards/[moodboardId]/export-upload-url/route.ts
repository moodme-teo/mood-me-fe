...
import { createExportUploadTarget } from "@/lib/moodboard/create-export-upload-target";
import { checkMoodboardOwner } from "@/lib/moodboard/moodboard-owner";

// 크롭 편집 완료 시 export한 PNG를 Supabase Storage에 직접 업로드하기 위한 signed
// upload URL을 발급한다. base64 dataURL을 이 서버로 보내지 않는 게 핵심이다 — 그러면
// Vercel 요청 바디 제한에 걸려 배포 환경에서만 나는 413("content too large")을 그대로
// 재현한다(#163).
export async function POST(
  ...
  const target = await createExportUploadTarget(moodboardId);
  if (!target.ok) {
